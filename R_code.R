...
###Clustering analysis
######################
sleepwork <- read.csv("Prob of rest for all subjects work.csv", header=TRUE)
sleepfree <- read.csv("Prob of rest for all subjects free.csv", header=TRUE)
sleepworkNS <- sleepwork[which(sleepwork$type=="Night"),]#63
sleepfreeNS <- sleepfree[which(sleepfree$type=="Night"),]
sleepworkDS <- sleepwork[which(sleepwork$type=="Other"),]#77
sleepfreeDS <- sleepfree[which(sleepfree$type=="Other"),]
sleepNS <- cbind(sleepworkNS[,3:290],sleepfreeNS[,3:290])
sleepDS <- cbind(sleepworkDS[,3:290],sleepfreeDS[,3:290])
dataNS <- as.matrix(sleepNS)
dataDS <- as.matrix(sleepDS)
...
parameter$workRI_logodd <- log(parameter$Workday_RI/(1-parameter$Workday_RI))
parameter$freeRI_logodd <- log(parameter$Freeday_RI/(1-parameter$Freeday_RI))

##Determine the marginal distribution of log odds of RI (Gaussian looks fine)
EnvStats::qqPlot(parameter$workRI_logodd, distribution = "norm",estimate.params=T,add.line = TRUE)
EnvStats::qqPlot(parameter$freeRI_logodd, distribution = "norm",estimate.params=T,add.line = TRUE)
